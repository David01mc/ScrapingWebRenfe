...
Uso:
  ...
  python renfe_asturias_cercanias.py --loop 30        # Captura cada 30s
...
def _get_first_position_captured_at(conn, vehicle_id, lat, lon):
    ...
    en las coordenadas (lat, lon). Permite calcular cuánto tiempo llevaba el tren
    en esa posición antes de moverse, compensando la baja frecuencia de actualización
    GPS de Renfe en cercanías.
    """
    cursor = conn.cursor()
    cursor.execute("""
...
def process_vehicle_positions(conn):
    ...
    for entity in data.get("entity", []):
        ...
        if prev:
            ...
            if pos_changed:
                # Buscar la primera vez que capturamos la posición anterior.
                # Así el delta de tiempo refleja cuánto llevaba el tren realmente ahí,
                # compensando que Renfe solo actualiza GPS cada 2-10 min en cercanías.
...
def capture_once(conn):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT DISTINCT trip_id FROM asturias_vehicle_snapshots
